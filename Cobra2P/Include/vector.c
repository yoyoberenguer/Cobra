/* C implementation */
/*
gcc -O2 -fomit-frame-pointer -o vector vector.c
gcc -ffast-math -O3 -fomit-frame-pointer -o vector vector.c
*/
...
#define M_PI2 3.14159265358979323846/2.0f
...
{
 float rad;
 rad=(float)(((float)atan2(v->y, v->x) * (float)RAD_TO_DEG + deg) * (float)DEG_TO_RAD);
 v->x = (float)cos(rad);
 v->y = (float)sin(rad);
}
...
float angle_to(struct vector2d v1, struct vector2d v2)
{
 ...
 return (float)((float)(v2_rad - v1_rad) * (float)RAD_TO_DEG);
}
...
{
 return (float)((float)atan2(v.y, v.x) * (float)RAD_TO_DEG);
}
...
//printf("\nTotal time for %i iterations : %f ", n, time_spent/(float)n);
//
//return 0;
//}