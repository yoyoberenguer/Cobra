...
cdef blur5x5_array24_inplace_c(unsigned char [:, :, :] rgb_array_, mask=None):
    """
    # Gaussian kernel 5x5
        # |1   4   6   4  1|
        # |4  16  24  16  4|
...
@cython.cdivision(True)
@cython.profile(False)
cdef blur5x5_surface24_inplace_c(surface_, mask=None):
    """
    # Gaussian kernel 5x5
        # |1   4   6   4  1|
        # |4  16  24  16  4|
...
cdef canny_blur5x5_surface24_c(surface_):
    ...
    assert isinstance(surface_, Surface), 'Argument image must be a' \
                                          ' valid Surface, got %s ' % type(surface_)
...
cdef canny_blur5x5_surface32_c(surface_):
    ...
    try:
        ...
        raise ValueError('\nInvalid texture or image. This version is'
                         ' compatible with 32-bit image format '
